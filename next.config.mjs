...
const nextConfig = {
  // === Android APK (Capacitor) 打包配置 ===
  // 静态导出 HTML/JS 到 out/，Capacitor 把 out/ 复制进 Android WebView，
  // 实现真正的本地 APP（不依赖浏览器/远程站点，根除 TWA 白屏问题）。
  output: "export",
  // trailingSlash 让静态导出生成 /index.html，Capacitor WebView 加载 capacitor://localhost/ 时能命中。
  trailingSlash: true,
  // 静态导出不支持 next/image 优化，关闭以避免运行时 _next/image 调用。
  images: {
    unoptimized: true,
  },
  // 静态导出需要确定的 buildId（避免每次构建后路径变化）。
  generateBuildId: () => "cap-android",
  typedRoutes: true,
